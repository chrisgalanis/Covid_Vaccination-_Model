...
def Solve_Model():
    ...
    for z in range(0,len(DateList)):
        T[z] = int(DateList[z])
        S[z] = int(PopulationList[z])
        I[z] = int (CaseList[z])
        R[z] = int(RecoveredList[z])
        
    print(S)
     #ax1.errorbar(x, y, yerr= sigs, markersize=8,fmt='o',color='k',mfc='white',mec='k', elinewidth=2, capsize=5, mew=0.9,zorder=5)  
    plt.plot(T, S,'bo',label = 'Susceptible Data')#You have to make another time array for dots
    plt.plot(T, R,'ro', label = 'Recovered Data')
    plt.plot(T, I,'go', label = 'Infected Data')
...
#Function write in Data.csv the old and new Data 
def Write(): 
    with open('C:/Users/chris/OneDrive/CovidAlgo/SIRV-Model/Data.csv','w', encoding='UTF-8',newline='') as csv_file:
        ...
        for i in range(0,len(CaseList)):
            writer.writerow([CaseList[i]]+[PopulationList[i]]+[RecoveredList[i]]+[DateList[i]])
...
DateList.append(int(DateList[-1])+1)
